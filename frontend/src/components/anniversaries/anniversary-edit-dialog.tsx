...
import { updateAnniversary, AnniversaryData } from "@/lib/api";

interface AnniversaryEditDialogProps {
  open: boolean;
  onClose: () => void;
  onUpdated: (anniversary: AnniversaryData) => void;
  anniversary: AnniversaryData | null;
}

export function AnniversaryEditDialog({
  open,
  onClose,
  onUpdated,
  anniversary,
}: AnniversaryEditDialogProps) {
  ...
  useEffect(() => {
    if (open && anniversary) {
      titleRef.current && (titleRef.current.value = anniversary.title);
      descRef.current && (descRef.current.value = anniversary.description || "");
      setAnniversaryDate(anniversary.date);
      setRepeatRule(anniversary.repeat_rule || "yearly");
      setScope(anniversary.scope || "shared");
      setError("");
    }
  }, [open, anniversary]);
  /* eslint-enable react-hooks/set-state-in-effect */

  if (!open || !anniversary) return null;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!anniversary.id) return;

    try {
      setSubmitting(true);
      setError("");
      const data: Partial<AnniversaryData> = {
        title,
      ...
      const updated = await updateAnniversary(anniversary.id, data);
      onUpdated(updated);
      handleClose();
    } catch (err) {
      setError("修改失败，请重试");
      ...
      <div className="relative z-10 bg-background border border-border rounded-xl shadow-lg w-[90vw] max-w-md mx-4 animate-fade-in-up">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-border">
          <div className="flex items-center gap-2.5">
            ...
            <h2 className="text-base font-semibold text-foreground">编辑纪念日</h2>
          </div>
          <button
            onClick={handleClose}
